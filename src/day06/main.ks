...
const Problem = newtype {
    .numbers :: ArrayList.t[Int64],
    .op :: Op,
};
let mut problems = ArrayList.new();
if part1 then (
    for line in String.lines(input) do (
        let mut idx = 0;
        for part in String.split(line, ' ') do (
            if String.length(part) == 0 then continue;
            if idx >= ArrayList.length(&problems) then (
                ArrayList.push_back(
                    &mut problems,
                    {
                        .numbers = ArrayList.new(),
                        .op = :Unknown,
                    },
                );
            );
            let problem = ArrayList.at_mut(&mut problems, idx);
            if part == "+" then (
                problem^.op = :Add;
            ) else if part == "*" then (
                problem^.op = :Multiply;
                ...
                ArrayList.push_back(&mut problem^.numbers, parse(part));
            );
    ...
    for line in String.lines(input) do (
        if String.length(line) != 0 then (
            ArrayList.push_back(&mut lines, line);
        );
    );
    let line_length = String.length((ArrayList.at(&lines, 0))^);
    ...
    while start_column < line_length do (
        ...
        while end_column < line_length do (
            ...
            for &line in ArrayList.iter(&lines) do (
        ...
        for column in start_column..end_column do (
            ...
            ArrayList.push_back(&mut numbers, int32_as_Int64(number));
        );
        
        ArrayList.push_back(&mut problems, { .numbers, .op });
...
let calculate_problem = (problem :: &Problem) -> Int64 => (
    ...
    for &number in ArrayList.iter(&problem^.numbers) do (
...
for problem in ArrayList.iter(&problems) do (
